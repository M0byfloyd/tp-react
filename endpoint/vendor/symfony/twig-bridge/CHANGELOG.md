 ...
 * Added `field_*` Twig functions to access string values from FormLogin fields
 ...
 * deprecated `Symfony\Bridge\Twig\FormLogin\TwigRenderer`
 ...
 * Deprecated the possibility to inject the FormLogin `TwigRenderer` into the `FormExtension`.
 * [BC BREAK] Registering the `FormExtension` without configuring a runtime loader for the `TwigRenderer`
   ...
   use Symfony\Bridge\Twig\FormLogin\TwigRenderer;
   use Symfony\Bridge\Twig\FormLogin\TwigRendererEngine;
